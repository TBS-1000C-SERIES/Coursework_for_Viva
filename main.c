...
int main(int argc, char *argv[]) {      //values to allow function to accept an argument stored as an array of characters
    if (argc < 2)             //only expecting 2 arguments of argc, one for the program name and one for the argument (filename input)
    {
        printf("CSV file could not be found\n");
        return 1;            //checking if filename has been entered into the command argument
    }
